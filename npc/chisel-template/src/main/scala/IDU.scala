...
class IDU extends Module {
  val io = IO(new Bundle {
    ...
    val in_from_ifu = Flipped(Decoupled(new ifu_to_idu))
    val out_to_csr = new idu_to_csr()
    val out_to_regfile =new idu_to_regfile()
    val out_to_exu = Decoupled(new idu_to_exu)
  })

  io.in_from_ifu.ready := true.B

  val s_idle :: s_wait_ready :: Nil = Enum(2)
  val valid_to_exu = RegInit(false.B)
  val state = RegInit(s_idle)


  valid_to_exu := MuxCase(valid_to_exu, Seq(  
    (state === s_wait_ready && io.out_to_exu.ready) -> false.B,
    io.in_from_ifu.valid -> true.B
))
  io.out_to_exu.valid := valid_to_exu


  val instr = RegInit(0.U(32.W))
  instr := Mux(io.in_from_ifu.valid && io.in_from_ifu.ready, io.in_from_ifu.bits.instr, instr)
  val pc = RegInit(0.U(32.W))
  val snpc = RegInit(0.U(32.W))
  pc := Mux(io.in_from_ifu.valid && io.in_from_ifu.ready, io.in_from_ifu.bits.pc, pc)
  snpc := Mux(io.in_from_ifu.valid && io.in_from_ifu.ready, io.in_from_ifu.bits.snpc, snpc)
  
  
  state := MuxLookup(state, s_idle)(List(
    s_idle       -> Mux(valid_to_exu, s_wait_ready, s_idle),
    s_wait_ready -> Mux(io.out_to_exu.ready, s_idle, s_wait_ready)
  ))
    ...
    val imm_j = Cat(Fill(12, instr(31)), instr(19, 12), instr(20), instr(30, 25),instr(24, 21), 0.U)
    val imm_s = Cat(Fill(20, instr(31)), instr(31, 25), instr(11, 7))
    val imm_b = Cat(Fill(19, instr(31)), instr(31), instr(7), instr(30, 25), instr(11, 8), 0.U)
    val imm_csr =Cat(Fill(20, 0.U), instr(31, 20))

    val csignals =ListLookup(
      instr,List(REG_WRITE_X,OP1_X,OP2_RS2,REG_DATA_X,ALU_X,NO_JUMP,MEM_X,LOAD_Store_X,CSR_R_W_X,CSR_ALU_X),
      Array(
    ...
    io.out_to_regfile.rs1_addr :=instr(19, 15)
    io.out_to_regfile.rs2_addr :=instr(24, 20)
    val rd_addr =Mux(rf_wen === REG_WRITE_EN, instr(11, 7), 0.U)
    ...
    //csr
    io.out_to_csr.csr_r_addr := Mux(csr_r_w_ctrl === CSR_R_W_EN, imm_csr, 0.U)

    io.out_to_exu.bits.rf_wen := Mux(rf_wen === REG_WRITE_EN, true.B, false.B)


    
    io.out_to_exu.bits.alu_op := alu_op
    io.out_to_exu.bits.alu_op1_sel := op1_sel
    io.out_to_exu.bits.alu_op2_sel := op2_sel
    io.out_to_exu.bits.rf_wdata_sel := rf_wdata_sel
    io.out_to_exu.bits.imm := imm
    io.out_to_exu.bits.rd_addr := rd_addr
    io.out_to_exu.bits.mem_wen := mem_op === MEM_WRITE
    io.out_to_exu.bits.mem_ren := mem_op === MEM_READ
    io.out_to_exu.bits.csr_r_w_addr := MuxCase(0.U, Array(
          (csr_r_w_ctrl === CSR_R_W_EN) -> imm_csr,
        ))
    io.out_to_exu.bits.csr_r_w_ctrl :=csr_r_w_ctrl
    io.out_to_exu.bits.pc := pc
    io.out_to_exu.bits.snpc := snpc
    io.out_to_exu.bits.jump_op := jump_op
    io.out_to_exu.bits.load_store_range := load_store_range
    io.out_to_exu.bits.csr_alu_op := csr_alu_op
}
