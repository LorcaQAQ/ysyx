...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  __VA_ARGS__ ; \
	...
	INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw,    I, R(rd) = Mr(src1 + imm, 4));//changed in mul-longlong
	...
	INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh,    I, R(rd) = Mr(src1+imm, 2); R(rd)=SEXT(R(rd), 16));
	...
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div,  R, if(src2==0) 
																	R(rd)= -1;
																else if(src1==0x80000000&&(int32_t)src2==-1) 
																	R(rd)= (int32_t)src1;
																else 
    ...
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, R(rd)=SEXT((int32_t)src1,64)*SEXT((int32_t)src2,64)>>32);//changed in mul-longlong
	INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu, R, if(src2==0) R(rd)=(int32_t)src1; else R(rd) = src1 % src2);
