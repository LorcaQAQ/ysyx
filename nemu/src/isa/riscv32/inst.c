...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  __VA_ARGS__ ; \
	...
	INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu, I, R(rd) = src1 < imm);
  	...
  	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai,  I, R(rd) = BITS(src1, 31,31) << (31-BITS(imm, 4,0)) | src1 >> BITS(imm, 4,0));
    ...
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, R(rd) = SEXT(BITS((int64_t)src1 * (int64_t)src2, 63,32),32));
