...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  __VA_ARGS__ ; \
	...
	INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli,  I,  if(BITS(imm, 5, 5)==0) R(rd) = src1 >> BITS(imm, 5,0));
    INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli,  I,  if(BITS(imm, 5, 5)==0) R(rd) = src1 << BITS(imm, 5,0));
