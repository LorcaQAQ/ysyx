...
static Token tokens[65532] __attribute__((used)) = {};//change from 32 to 66532
	...
	for (int i = q; i > p; i--)
	{
		...
		if (mark == 0) {
			if (tokens[i].type == OR) {//the token is &&,||
				...
				if (position == q || tokens[position].type == '+' || tokens[position].type == '-' || tokens[position].type == '*' || tokens[position].type == '/')
					position = i;
			}
			else if (tokens[i].type == '+' || tokens[i].type == '-') {
				//the tokens is +,-, and it is not within parenthese.
				if (position == q || tokens[position].type == '*' || tokens[position].type == '/')
					position = i;
			}
			else if (tokens[i].type == '*' || tokens[i].type == '/') {
